...
enum BLOSC_VERSION_MINOR = 2;    /* for minor interface/format changes  */
enum BLOSC_VERSION_RELEASE = 3;  /* for tweaks, bug-fixes, or development */

enum BLOSC_VERSION_STRING = "1.2.3";  /* string version.  Sync with above! */
enum BLOSC_VERSION_REVISION = "$Rev$";   /* revision version */
enum BLOSC_VERSION_DATE = "$Date:: 2013-05-17 #$";    /* date version */


/* The *_FORMAT symbols should be just 1-byte long */
enum BLOSC_VERSION_FORMAT = 2;   /* Blosc format version, starting at 1 */
enum BLOSCLZ_VERSION_FORMAT = 1; /* Blosclz format version, starting at 1 */

/* The combined blosc and blosclz formats */
enum BLOSC_VERSION_CFORMAT = (BLOSC_VERSION_FORMAT << 8) & (BLOSCLZ_VERSION_FORMAT);
