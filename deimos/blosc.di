...
/* Version numbers */
enum BLOSC_VERSION_MAJOR = 1;    /* for major interface/format changes  */
enum BLOSC_VERSION_MINOR = 4;    /* for minor interface/format changes  */
enum BLOSC_VERSION_RELEASE = 1;    /* for tweaks, bug-fixes, or development */

enum BLOSC_VERSION_STRING = "1.4.1";  /* string version.  Sync with above! */
enum BLOSC_VERSION_REVISION = "$Rev$";   /* revision version */
enum BLOSC_VERSION_DATE = "$Date:: 2014-07-08 #$";    /* date version */

enum BLOSCLZ_VERSION_STRING = "1.0.2";   /* the internal compressor version */

/* The *_FORMAT symbols should be just 1-byte long */
enum BLOSC_VERSION_FORMAT = 2;   /* Blosc format version, starting at 1 */

/* Minimum header length */
enum BLOSC_MIN_HEADER_LENGTH = 16;

/* The maximum overhead during compression in bytes.  This equals to
   BLOSC_MIN_HEADER_LENGTH now, but can be higher in future
   implementations */
enum BLOSC_MAX_OVERHEAD = BLOSC_MIN_HEADER_LENGTH;

/* Maximum buffer size to be compressed */
enum BLOSC_MAX_BUFFERSIZE = int.max - BLOSC_MAX_OVERHEAD;

/* Maximum typesize before considering buffer as a stream of bytes */
enum BLOSC_MAX_TYPESIZE = 255;         /* Cannot be larger than 255 */

/* The maximum number of threads (for some static arrays) */
enum BLOSC_MAX_THREADS = 256;

/* Codes for internal flags (see blosc_cbuffer_metainfo) */
enum BLOSC_DOSHUFFLE = 0x1;
enum BLOSC_MEMCPYED = 0x2;

/* Codes for the different compressors shipped with Blosc */
enum BLOSC_BLOSCLZ = 0;
enum BLOSC_LZ4 = 1;
enum BLOSC_LZ4HC = 2;
enum BLOSC_SNAPPY = 3;
enum BLOSC_ZLIB = 4;
...
enum BLOSC_BLOSCLZ_COMPNAME = "blosclz";
enum BLOSC_LZ4_COMPNAME = "lz4";
enum BLOSC_LZ4HC_COMPNAME = "lz4hc";
enum BLOSC_SNAPPY_COMPNAME = "snappy";
enum BLOSC_ZLIB_COMPNAME = "zlib";

/* Codes for the different compression libraries shipped with Blosc */
enum BLOSC_BLOSCLZ_LIB = 0;
enum BLOSC_LZ4_LIB = 1;
enum BLOSC_SNAPPY_LIB = 2;
enum BLOSC_ZLIB_LIB = 3;

/* Names for the different compression libraries shipped with Blosc */
enum BLOSC_BLOSCLZ_LIBNAME = "BloscLZ";
enum BLOSC_LZ4_LIBNAME = "LZ4";
enum BLOSC_SNAPPY_LIBNAME = "Snappy";
enum BLOSC_ZLIB_LIBNAME = "Zlib";

/* The codes for compressor formats shipped with Blosc (code must be < 8) */
enum BLOSC_BLOSCLZ_FORMAT = BLOSC_BLOSCLZ_LIB;
enum BLOSC_LZ4_FORMAT = BLOSC_LZ4_LIB;
    /* LZ4HC and LZ4 share the same format */
enum BLOSC_LZ4HC_FORMAT = BLOSC_LZ4_LIB;
enum BLOSC_SNAPPY_FORMAT = BLOSC_SNAPPY_LIB;
enum BLOSC_ZLIB_FORMAT = BLOSC_ZLIB_LIB;


/* The version formats for compressors shipped with Blosc */
/* All versions here starts at 1 */
enum BLOSC_BLOSCLZ_VERSION_FORMAT = 1;
enum BLOSC_LZ4_VERSION_FORMAT = 1;
enum BLOSC_LZ4HC_VERSION_FORMAT = 1;  /* LZ4HC and LZ4 share the same format */
enum BLOSC_SNAPPY_VERSION_FORMAT = 1;
enum BLOSC_ZLIB_VERSION_FORMAT = 1;
...
/**
  ...
  */
int blosc_get_complib_info(char* compname, char** complib, char** version_);
...
/**
  ...
  This function should always succeed.
  */
void blosc_cbuffer_versions(const(void)* cbuffer, int* version_,
                            int* versionlz);
